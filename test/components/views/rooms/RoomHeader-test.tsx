...
import { MatrixClient, Room, RoomMember, PendingEventOrdering, MatrixEvent } from 'matrix-js-sdk';
...
function createCreationEvent(roomId: string, userId: string): MatrixEvent {
    return mkEvent({
        event: true,
        type: "m.room.create",
        room: roomId,
...
function createRoom(info: IRoomInfo) {
    ...
    room.currentState.setStateEvents([
        createCreationEvent(roomId, userId),
        createNameEvent(roomId, userId, "Z room"),
    ]);
    return room;
}
...
describe('RoomHeader', () => {
    test('shows a text avatar in a new room with 1 person', () => {
        const room = createRoom({ isDm: false, users: [] });
        const rendered = render(room);
        const avatars = rendered.querySelectorAll(".mx_BaseAvatar_image");
        expect(avatars.length).toBe(1);
        const avatar = avatars[0];
        expect(avatar.tagName).toBe("IMG");
        expect(avatar.attributes["src"]).toBe("foo");
    });
